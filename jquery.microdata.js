...
(function ($) {
  ...
  var
    ...
    microdataCurieDefaults = $.fn.curie.defaults,
...
//            atts[a.nodeName] = a.nodeValue === '' ? undefined : a.nodeValue;
          } else if (/itemref|lang|xml:lang|itemid|href|src|itemprop|itemtype|content|datatype/.test(a.nodeName)) {
        ...
        while (a !== null) {
          ...
          } else if (/itemid|href|src|itemref|itemprop|itemtype|content|datatype|lang|xml:lang/.test(name)) {
...
//      }
    ...
    getObjectResource = function (elem, context, relation) {
      ...
      r = relation === undefined ? atts.itemref !== undefined : relation;
      resource = atts.itemref;
    ...
    getSubject = function (elem, context, relation) {
      ...
      r = relation === undefined ? atts.itemref !== undefined : relation;
      if (atts.itemid !== undefined) {
        subject = resourceFromSafeCurie(atts.itemid, elem, curieOptions);
      }
      if (subject === undefined && atts.src !== undefined) {
        subject = resourceFromSafeCurie(atts.src, elem, curieOptions);
      }
      if (!r && subject === undefined && atts.itemref !== undefined) {
        subject = resourceFromSafeCurie(atts.itemref, elem, curieOptions);
      }
    ...
    removeMicrodata = function (what) {
      var span, atts, property, rel, type,
        ns = this.xmlns();
      atts = getAttributes(this).atts;
      if (what.length) {
      ...
      hasRelation = atts.itemref !== undefined;
      hasMicrodata = hasRelation || atts.itemprop !== undefined || atts['itemtype'] !== undefined;
      if (hasMicrodata) {
        if (what.itemprop !== undefined) {
            if (atts.itemprop !== undefined) {
                property = removeCurie(atts.itemprop, what.itemprop, {
                    namespaces: ns
                });
                if (property === '') {
                    ...
                    this.attr('itemprop', property);
                }
            }
            if (atts.itemref !== undefined) {
                rel = removeCurie(atts.itemref, what.itemprop, {
                    namespaces: ns
                });
                if (rel === '') {
                    this.removeAttr('itemref');
                }
                else {
                    this.attr('itemref', rel);
                }
            }
        }
        if (what.itemtype !== undefined) {
          if (atts['itemtype'] !== undefined) {
            type = removeCurie(atts['itemtype'], what.itemtype, { namespaces: ns });
            if (type === '') {
              this.removeAttr('itemtype');
            } else {
              this.attr('itemtype', type);
            }
          }
        }
        if (atts.property === this.attr('itemprop') && atts.itemref === this.attr('itemref') && atts['itemtype'] === this.attr('itemtype')) {
   ...
   *  span.microdata('&lt;> dc:date "2008-10-19"^^xsd:date .');
   */
  ...
  /**
   * Removes the specified microdata markup from each of the items in the current jQuery selection. The input parameter can be either an object or an array of objects. The objects can either have a <code>type</code> property, in which case the specified type is removed from the microdata provided on the selected elements, or a <code>property</code> property, in which case the specified property is removed from the microdata provided on the selected elements.
   * @methodOf jQuery#
   ...
   * $('#main > p > a').removeMicrodata({ itemprop: "dc:creator" });
   * @example
   * // To remove a type from an element
   * $('#main >p > a').removeMicrodata({ itemtype: "foaf:Person" });
   * @example
   * // To remove multiple triples from an element
   * $('#main > p > a').removeMicrodata([{ itemprop: "foaf:depicts" }, { itemprop: "dc:creator" }]);
   */
